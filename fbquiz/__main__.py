...
for line in correct_answers:
    if re.match('\d\s\s\d', line):
        correct_answers[correct_answers.index(line)] = line.replace(' ' * 2, ' ? ')
...
def main():
    ...
    for i in range(0, num_of_questions):
        ...
        if not i + 1 == num_of_questions:
            next_question_i = content.index(
                list(filter(lambda a: f'0{i + 2})' in a, content[current_question_i + 2:]))[0])
        else:
